...
#[test]
fn same_output_as_lib() {
    let mut key = [0; KEY_LEN];
    baokeshed::test_shared::paint_test_input(&mut key);

    for &case in baokeshed::test_shared::TEST_CASES {
        dbg!(case);
        let mut input = vec![0; case];
        baokeshed::test_shared::paint_test_input(&mut input);

        // all at once
        let mut hasher = Hasher::new();
        hasher.update(&input);
        let output = hasher.finalize();
        assert_eq!(baokeshed::hash(&input), output);
        ...
        hasher.update(&input);
        let output = hasher.finalize();
        assert_eq!(baokeshed::keyed_hash(&input, &key), output);

        // derive key
        let mut hasher = Hasher::new_derive_key(&key);
        hasher.update(&input);
        let output = hasher.finalize();
        assert_eq!(baokeshed::derive_key(&key, &input), output);
        ...
        let mut xof = baokeshed::keyed_hash_xof(&input, &key);
